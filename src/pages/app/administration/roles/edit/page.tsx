...
export default function EditRolePage() {
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            const canEditFresh = rolePermissions?.EditPermission === 1;
    ...
    useEffect(() => {
        if (location.state?.roleData) {
            ...
            // Fallback: Fetch from API list if state is missing (e.g., direct URL access)
            const fetchRoleFromList = async () => {
                try {
                    const baseUrl = import.meta.env.VITE_BASE_URL;
                    ...
                    // We might not be able to filter by ID directly in this API based on previous usage, 
                    // so we might need to fetch all and find. 
                    // Or if we can't efficiently, we might just leave it empty or show error.
                    // For now, let's try to fetch list and find by ID if possible, 
                    // or just rely on the state as the primary method requested by user.
                    ...
                    if (data.status === "SUCCESS" && data.data?.RecordListing) {
                        const role = data.data.RecordListing.find((r: any) => r.TableID == id);
                        if (role) {
                            setInitialValues({
                    ...
                    console.error("Failed to fetch role details fallback", e);
                }
            };
            fetchRoleFromList();
        }
            ...
            <Grid container spacing={2.5} className="w-full" size={12}>
                <Grid size={{ xs: 12, md: "grow" }}>
                    ...
                    <Breadcrumbs>
                        ...
                        <Typography variant="body2">Edit Role</Typography>
                    </Breadcrumbs>
                </Grid>
