...
import { Plus, Trash2, GripVertical, ArrowLeft, Eye, Send, Globe, Settings2, FileText, Link2, CheckCircle2, LayoutPanelTop, Wand2, MonitorSmartphone } from "lucide-react";
...
const LIST_THEMES_API_URL = `${import.meta.env.VITE_SURVEY_API_URL}api/theme/list_themes.php`;
...
function buildSurveyPayload(survey: SurveyState, sections: Section[]): SaveSurveyPayload {
  return {
    survey: {
      id: survey.id,
    ...
    sections: sections.map((section, sectionIndex) => ({
      ...
      questions: (section.questions || []).map((question, questionIndex) => ({
        ...
        options: (question.options || []).map((option, optionIndex) => ({
          ...
          option_score: option.score === null || option.score === undefined ? null : Number(option.score),
...
async function saveSurveyApi(payload: SaveSurveyPayload, token: string): Promise<any> {
...
async function fetchThemes(): Promise<ThemeListApiResponse> {
  const response = await fetch(LIST_THEMES_API_URL, {
    method: "GET",
...
export default function SurveyCreationScreenAllInOne() {
  ...
  useEffect(() => {
    async function loadMasters() {
      try {
        ...
        const [mastersResult, themesResult] = await Promise.all([fetchMasters(), fetchThemes()]);
        setDevices(mastersResult.data.devices || []);
        setThemes((themesResult.data.themes || []).filter((theme) => Number(theme.is_active) === 1));
      } catch (error: any) {
        toast.error(error.message || "Failed to load survey setup data.");
      } finally {
  ...
  const handleThemeChange = async (value: string) => {
    ...
    if (!Number.isFinite(nextThemeId) || nextThemeId <= 0) {
      setSelectedTheme(null);
      setSurvey((prev) => ({ ...prev, themeId: null }));
      return;
    }

    setSurvey((prev) => ({ ...prev, themeId: nextThemeId }));

    try {
      setThemesLoading(true);
      const themeDetails = await fetchThemeById(nextThemeId);
      setSelectedTheme(themeDetails);
    } catch (error: any) {
      toast.error(error.message || "Failed to load selected theme.");
      setSelectedTheme(null);
    } finally {
      setThemesLoading(false);
    }
  ...
  const readiness = Math.min(100, 30 + (survey.name ? 10 : 0) + (survey.deviceIds.length ? 15 : 0) + (sections.length ? 15 : 0) + (totalQuestions ? 10 : 0) + ((survey.publishWeb || survey.publishQR || survey.publishKiosk || survey.publishEmail) ? 20 : 0));
        ...
        <Card className="rounded-[28px] border-0 shadow-sm">
          ...
          <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="space-y-2 xl:col-span-2">
              <Label>Theme</Label>
              <Select
                value={survey.themeId ? String(survey.themeId) : "none"}
                ...
                <SelectContent>
                  ...
                  {themes.map((theme) => (
                    <SelectItem key={theme.id} value={String(theme.id)}>
                      {theme.theme_name} ({theme.layout_mode === "multi_page" ? "Multi Page" : "Single Page"})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              <div className="text-xs text-slate-500">
                {selectedTheme
                  ? `Selected theme layout: ${selectedTheme.layout_mode === "multi_page" ? "One question per page" : "All questions on one page"}`
                  : "Choose a theme to see its layout and styles in the survey preview."}
              </div>
            </div>
            ...
            <div className="space-y-2 xl:col-span-4">
              ...
              {errors.survey?.devices && <p className="text-sm text-red-600">{errors.survey.devices}</p>}
            </div>
              ...
              <TabsContent value="builder" className="mt-6">
                <div className="grid gap-6 xl:grid-cols-[320px_1fr]">
                  <Card className="rounded-3xl border-slate-200">
                    <CardHeader><CardTitle className="text-lg">Sections</CardTitle></CardHeader>
                    <CardContent className="space-y-3">
                      {sections.length === 0 ? <div className="rounded-2xl border border-dashed p-6 text-center text-slate-500">No sections added yet.</div> : null}
                  ...
                  <div className="space-y-6">
                    <Card className="rounded-3xl border-slate-200">
                      <CardHeader><CardTitle className="text-lg">Section Details</CardTitle></CardHeader>
                    ...
                    <Card className="rounded-3xl border-slate-200">
                      <CardHeader><CardTitle className="text-lg">Question Editor</CardTitle></CardHeader>
                            ...
                            {selectedQuestion ? (
                              <>
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    <Select value={selectedQuestion.type} onValueChange={(v) => updateQuestion(selectedQuestion.id, "type", v as QuestionType)}>
                                      ...
                                      <SelectContent>
                                        ...
                                        <SelectItem value="smiley_3">3 Smileys</SelectItem>
                                        <SelectItem value="smiley_5">5 Smileys</SelectItem>
                                ...
                                <div className="grid gap-3 md:grid-cols-2">
                                  <div className="rounded-2xl border p-4 flex items-center justify-between">
                                    <div><div className="font-medium">Required</div><div className="text-sm text-slate-500">Respondent must answer this question</div></div>
                                    <Switch checked={selectedQuestion.required} onCheckedChange={(v) => updateQuestion(selectedQuestion.id, "required", v)} />
                                  </div>
                                  <div className="rounded-2xl border p-4 flex items-center justify-between">
                                    <div><div className="font-medium">Scoring Enabled</div><div className="text-sm text-slate-500">Use this question in score calculations</div></div>
        ...
        <Card className="rounded-[28px] border-0 shadow-sm">
          ...
          <CardContent>
            <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
              <div className="space-y-6">
                <Card className="rounded-3xl border-slate-200">
                  <CardHeader><CardTitle className="text-lg">Channel Setup</CardTitle></CardHeader>
                ...
                <Card className="rounded-3xl border-slate-200">
                  <CardHeader><CardTitle className="text-lg">Publish Assets</CardTitle></CardHeader>
                  <CardContent className="grid gap-4 md:grid-cols-2">
              ...
              <Card className="rounded-3xl border-slate-900 bg-slate-950 text-white h-fit">
                ...
                <CardContent className="space-y-5">
                  ...
                  <div className="grid gap-3 grid-cols-3">
                    <div className="rounded-2xl bg-white/5 p-4">
                      <div className="text-sm text-slate-300">Devices</div>
                      <div className="text-2xl font-semibold mt-1">{survey.deviceIds.length}</div>
                    </div>
                    <div className="rounded-2xl bg-white/5 p-4">
                  ...
                  <div className="rounded-2xl bg-white/5 p-4 text-sm space-y-2">
                    ...
                    <div className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4 text-emerald-400" /> Devices selected</div>
