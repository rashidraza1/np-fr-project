...
function buildQueryString(filters: Filters) {
  ...
  if (filters.branch_name !== "all") params.set("branch_name", filters.branch_name);
  if (filters.department_name !== "all") params.set("department_name", filters.department_name);
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="rounded-[28px] bg-white border border-slate-200 p-6 shadow-sm space-y-5">
          <div>
            ...
            <p className="text-slate-500 mt-1">Search the survey master table and narrow the list using filters.</p>
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
            <div className="relative xl:col-span-2">
              ...
              <input
                ...
                placeholder="Search survey name, code, branch, department"
              />
            ...
            <select
              ...
              onChange={(e) => updateFilter("department_name", e.target.value)}
              className="rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm outline-none"
            >
                  ...
                  {rows.map((item) => (
                    <tr key={item.id} className="border-t border-slate-100 bg-white">
                      ...
                      <td className="px-4 py-4">
                        ...
                        <div className="text-xs text-slate-500 mt-1">{item.department_name || "-"}</div>
                      </td>
                      <td className="px-4 py-4">
              ...
              {rows.map((item) => (
                <div key={item.id} className="rounded-[22px] border border-slate-200 bg-white p-5 shadow-sm space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <span className="rounded-full bg-slate-50 text-slate-700 border border-slate-200 px-3 py-1 text-xs font-bold">
                      {item.branch_name || "-"}
                    </span>
                    <span className="rounded-full bg-slate-50 text-slate-700 border border-slate-200 px-3 py-1 text-xs font-bold">
