...
type MasterApiResponse = {
  success: boolean;
  message?: string;
  data: { branches: MasterItem[]; departments: MasterItem[] };
};
...
function validateSurveyForm(
  ...
  if (!survey.department?.trim())
    errors.survey!.department = "Department is required.";
  ...
  sections.forEach((section) => {
    ...
    if (!section.questions || section.questions.length === 0)
      sectionError.section.questions =
        "At least one question is required in each section.";
...
function buildSurveyPayload(
  ...
  return {
    survey: {
      ...
      survey_type: (survey.type || "custom") as
        | "nps"
        | "csat"
        | "ces"
        | "custom",
...
function mapApiToSurveyState(api: GetSurveyApiResponse["data"]): {
  ...
  const survey: SurveyState = {
    ...
    branch: api.survey.branch_name || "",
    department: api.survey.department_name || "",
    ...
    <div className="rounded-3xl border bg-white shadow-sm overflow-hidden">
      <div className="bg-slate-950 text-white p-6">
        ...
        <div className="text-sm text-slate-300 mt-2">
          {survey.description || "Survey description will appear here."}
        </div>
      </div>
            ...
            <CardContent className="space-y-4">
              {section.questions.map((q, qi) => (
                <div key={q.id} className="rounded-2xl border p-4 space-y-3">
                  ...
                  {q.helpText && (
                    <div className="text-sm text-slate-500">{q.helpText}</div>
                  )}
                </div>
              ))}
            </CardContent>
  ...
  useEffect(() => {
    async function loadPage() {
      ...
      try {
        ...
        setBranches(masters.data.branches || []);
        setDepartments(masters.data.departments || []);
        ...
        <Card className="rounded-[28px] border-0 shadow-sm">
          ...
          <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                value={survey.branch}
                onValueChange={(v) => updateSurvey("branch", v)}
              ...
              {errors.survey?.branch && (
                <p className="text-sm text-red-600">{errors.survey.branch}</p>
              )}
            </div>
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(v) => updateSurvey("department", v)}
                disabled={mastersLoading}
              >
                <SelectTrigger>
                  <SelectValue
                  ...
                  <Card className="rounded-3xl border-slate-200">
                    ...
                    <CardContent className="space-y-3">
                      ...
                      {sections.map((section, i) => (
                        <div
                          key={section.id}
                          className={`rounded-2xl border p-3 ${selectedSectionId === section.id
                            ...
                            <ScrollArea className="pr-3">
                              <div className="space-y-3">
                                {selectedSection?.questions.map((q, i) => (
                                  <div
                                    key={q.id}
                                    className={`rounded-2xl border p-3 ${selectedQuestionId === q.id
                                      ? "border-slate-900 bg-slate-50"
                                      : "bg-white"
                                      }`}
                                  >
