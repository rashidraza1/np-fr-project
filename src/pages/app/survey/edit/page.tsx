...
type ThemeListApiResponse = {
  ...
  message?: string;
  data: {
    themes: Array<Pick<SurveyThemeConfig, "id" | "theme_name" | "theme_code" | "layout_mode" | "is_active">>;
  };
};
...
async function fetchThemes(): Promise<Array<Pick<SurveyThemeConfig, "id" | "theme_name" | "theme_code" | "layout_mode" | "is_active">>> {
  const response = await fetch(LIST_THEMES_API_URL, {
    method: "GET",
    headers: { "Content-Type": "application/json" },
  });
  const data: ThemeListApiResponse = await response.json();
  if (!response.ok || !data.success) {
    throw new Error(data.message || "Failed to load themes.");
  }
  const themes = Array.isArray(data.data?.themes) ? data.data.themes : [];
  return themes.filter((theme) => Number(theme.is_active) === 1 || theme.is_active === true);
}
...
export default function SurveyEditPage() {
  ...
  const [surveyThemes, setSurveyThemes] = useState<Array<Pick<SurveyThemeConfig, "id" | "theme_name" | "theme_code" | "layout_mode" | "is_active">>>([]);
  ...
  useEffect(() => {
    async function loadPage() {
      ...
      try {
        ...
        const [masters, surveyResult, themes] = await Promise.all([
          fetchMasters(),
  ...
  useEffect(() => {
    async function loadSelectedTheme() {
      if (!survey.themeId || survey.themeId <= 0) {
        setSelectedTheme(null);
        return;
      }

      try {
        const themeDetails = await fetchThemeById(survey.themeId);
        setSelectedTheme(themeDetails);
      } catch (error: any) {
  ...
  async function handleThemeChange(value: string) {
    const nextThemeId = value === "none" ? null : Number(value);
    if (!nextThemeId || nextThemeId <= 0) {
      setSelectedTheme(null);
  ...
  const updateSurvey = (key: keyof SurveyState, value: string | boolean | number[] | number | null) =>
        ...
        <Card className="rounded-[28px] border-0 shadow-sm">
          ...
          <CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="space-y-2">
              <Label>Theme</Label>
              <Select
                value={survey.themeId && survey.themeId > 0 ? String(survey.themeId) : "none"}
              ...
              {selectedTheme && (
                <div className="text-xs text-slate-500">
                  Selected theme: <span className="font-medium text-slate-700">{selectedTheme.theme_name}</span>
                </div>
              )}
            </div>
                                ...
                                <div className="grid gap-4 md:grid-cols-2">
                                  ...
                                  <div className="space-y-2">
                                    ...
                                    >
                                      ...
                                      <SelectContent>
                                        <SelectItem value="nps_0_10">NPS (1 to 10)</SelectItem>
                                        <SelectItem value="csat_1_5">CSAT (1 to 5)</SelectItem>
                                        <SelectItem value="ces_1_5">CES (1 to 5)</SelectItem>
