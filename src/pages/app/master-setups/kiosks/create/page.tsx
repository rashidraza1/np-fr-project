...
export default function CreateKioskPage() {
    const { canAdd } = usePermission("Kiosks");
    const { fetchMenuPermissions, getFeaturePermissions } = useLayoutContext();
    const navigate = useNavigate();
    const [loading, setLoading] = useState(false);

    const validationSchema = yup.object({
        kioskName: yup.string().trim().required("Title Name (English) is required"),
        kioskNameAr: yup.string().trim().required("Title Name (Arabic) is required"),
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            if (!canAddFresh) {
                toast.error("You do not have permission to add kiosks.");
                setTimeout(() => {
            ...
            try {
                ...
                const response = await fetch(
                    `${import.meta.env.VITE_BASE_URL}/webservice/?class=general&action=AddEditRSIGeneralMaster&WebServiceUserName=WebserviceUser&Password=oqkq12345234`,
                    {
                        ...
                        body: JSON.stringify({
                            Trigger: "add",
                            MasterType: 4,
                            TitleEnglish: values.kioskName,
                            TitleArabic: values.kioskNameAr,
                ...
                if (data.status === "SUCCESS") {
                    ...
                    toast.error(data.message || "Failed to create kiosk");
                }
            } catch (error) {
            ...
            <Grid container spacing={2.5} className="w-full" size={12}>
                <Grid size={{ xs: 12, md: "grow" }}>
                    ...
                    <Breadcrumbs>
                        ...
                        <Typography variant="body2">Add Kiosk</Typography>
                    </Breadcrumbs>
                </Grid>
                                ...
                                <Grid container spacing={2} alignItems="center">
                                    <Grid size={{ xs: 12, md: 4 }}>
                                        <FormControl
                                            ...
                                            size="small"
                                            fullWidth
                                            error={formik.touched.kioskName && Boolean(formik.errors.kioskName)}
                                        >
                                            ...
                                            <Input
                                                ...
                                                id="kioskName"
                                                name="kioskName"
                                                value={formik.values.kioskName}
                                                onChange={formik.handleChange}
                                                onBlur={formik.handleBlur}
                                            />
                                            {formik.touched.kioskName && formik.errors.kioskName && (
                                                <Typography variant="caption" color="error" className="mt-1">
                                                    {formik.errors.kioskName}
                                                </Typography>
                                            )}
                                        </FormControl>
                                    </Grid>
                                    <Grid size={{ xs: 12, md: 4 }}>
                                        <FormControl
                                            ...
                                            size="small"
                                            fullWidth
                                            error={formik.touched.kioskNameAr && Boolean(formik.errors.kioskNameAr)}
                                        >
                                            <FormLabel component="label">
                                            ...
                                            <Input
                                                ...
                                                id="kioskNameAr"
                                                name="kioskNameAr"
                                                value={formik.values.kioskNameAr}
                                                onChange={formik.handleChange}
                                                onBlur={formik.handleBlur}
                                                dir="rtl"
                                            />
                                            {formik.touched.kioskNameAr && formik.errors.kioskNameAr && (
                                                <Typography variant="caption" color="error" className="mt-1">
                                                    {formik.errors.kioskNameAr}
                                                </Typography>
                                            )}
                                        </FormControl>
                                    </Grid>
