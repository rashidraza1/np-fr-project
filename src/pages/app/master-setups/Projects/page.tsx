...
export default function Page() {
  const { canAdd, canEdit, canDelete } = usePermission("Projects");
    ...
    try {
      ...
      if (data.status === "SUCCESS" && data.data && data.data.RecordListing) {
        ...
        const mappedRows = listing.map((item: any) => ({
          id: item.TableID,
          projectName: item.TitleEnglish,
          projectNameArabic: item.TitleArabic,
  ...
  const handleExportExcel = () => {
    ...
    const dataToExport = visibleRows.map(row => ({
      ...
      "Project Name (Arabic)": row.projectNameArabic,
      "Status": row.status
    }));
    ...
    XLSX.writeFile(workbook, "Projects_List.xlsx", { bookType: 'xlsx', type: 'binary' });
  ...
  const handleExportPDF = () => {
    ...
    const tableColumn = ["Project Name", "Project Name (Arabic)", "Status"];
    const tableRows = visibleRows.map(row => [
      row.projectName,
      reshapeArabic(row.projectNameArabic),
      row.status
    ]);
        ...
        if (canEdit) {
          actions.push(
            <GridActionsCellItem
              ...
              label="Edit"
              showInMenu
              onClick={() => navigate(`/master-setups/projects/edit/${params.id}`, { state: params.row })}
            />
          );
        }
        if (canDelete) {
              ...
              {canAdd && (
                <Grid size={{ xs: 12, md: "auto" }} className="flex flex-row items-start gap-2">
                  <Tooltip title="Add Item">
                    <Button
                      component={Link}
                      to="/master-setups/projects/create"
                      className="surface-standard"
                      size="medium"
                      color="grey"
                      variant="surface"
            ...
            <Grid container spacing={5} className="w-full" size={12}>
              <Grid size={12}>
                <Box className="bg-white flex w-full flex-col items-start gap-4 rounded-2xl p-4 shadow-sm">
                  <Typography variant="h6" className="text-text-primary px-2">Project Search</Typography>
