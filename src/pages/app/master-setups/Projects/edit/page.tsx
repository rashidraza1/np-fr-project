...
export default function EditProjectPage() {
    const { canEdit } = usePermission("Projects");
    ...
    const validationSchema = yup.object({
        projectName: yup.string().trim().required("Title Name (English) is required"),
        projectNameAr: yup.string().trim().required("Title Name (Arabic) is required"),
                ...
                const response = await fetch(
                    `${import.meta.env.VITE_BASE_URL}/webservice/?class=general&action=AddEditRSIGeneralMaster&WebServiceUserName=WebserviceUser&Password=oqkq12345234`,
                    {
                        ...
                        body: JSON.stringify({
                            Trigger: "edit",
                            MasterType: 3,
                            TitleEnglish: values.projectName,
                            TitleArabic: values.projectNameAr,
    ...
    useEffect(() => {
        if (location.state) {
            const { projectName, projectNameArabic, status } = location.state;
            formik.setValues({
                projectName: projectName || "",
                projectNameAr: projectNameArabic || "",
            ...
            <Grid container spacing={2.5} className="w-full" size={12}>
                <Grid size={{ xs: 12, md: "grow" }}>
                    ...
                    <Breadcrumbs>
                        ...
                        <Link color="inherit" to="/master-setups/projects">
                            Projects
                        </Link>
                                ...
                                <Grid container spacing={2} alignItems="center">
                                    ...
                                    <Grid size={{ xs: 12, md: 4 }}>
                                        <FormControl
                                            ...
                                            size="small"
                                            fullWidth
                                            error={formik.touched.projectNameAr && Boolean(formik.errors.projectNameAr)}
                                        >
                                            <FormLabel component="label">
                                            ...
                                            <Input
                                                ...
                                                id="projectNameAr"
                                                name="projectNameAr"
                                                value={formik.values.projectNameAr}
                                                onChange={formik.handleChange}
                                                onBlur={formik.handleBlur}
                                                dir="rtl"
                                            />
                                            {formik.touched.projectNameAr && formik.errors.projectNameAr && (
                                                <Typography variant="caption" color="error" className="mt-1">
                                                    {formik.errors.projectNameAr}
                                                </Typography>
                                            )}
                                        </FormControl>
                                    </Grid>
