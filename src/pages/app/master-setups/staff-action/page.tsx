...
export default function StaffActionPage() {
  ...
  }, [filterLogType, startDate, endDate]);

  useEffect(() => {
    // No initial fetch to keep table hidden by default
  }, []);
  ...
  return (
    <Grid container spacing={5}>
      ...
      >
        <DialogTitle className="flex justify-between items-center pt-4 pb-2 border-b">
          <Box className="flex items-center gap-2">
            <NiEyeOpen size="medium" className="text-primary" />
            <Typography variant="h6">Staff Action Details - {selectedRow?.kioskName}</Typography>
          </Box>
          <IconButton onClick={() => setDetailsPopupOpen(false)} size="small">
            <NiCross size="medium" />
          </IconButton>
        </DialogTitle>
        <DialogContent className="bg-gray-50 pt-4">
          {loadingDetails ? (
            <Box className="flex justify-center items-center py-20"><CircularProgress /></Box>
          ) : (
            <Box className="flex flex-col gap-3">
              {/* Header Info */}
              <Box className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between mb-2">
                <Box className="flex flex-col">
                  <Typography variant="caption" className="text-gray-500 uppercase tracking-wider font-semibold">Staff Name</Typography>
                  <Typography variant="body1" className="text-gray-800 font-bold">{selectedRow?.staffName}</Typography>
                </Box>
                <Box className="flex flex-col items-end">
                  <Typography variant="caption" className="text-gray-500 uppercase tracking-wider font-semibold text-right">Log Type</Typography>
                  <Button size="tiny" color={selectedRow?.logType === '1' || selectedRow?.logType === '2' ? 'info' : selectedRow?.logType === '3' ? 'success' : selectedRow?.logType === '4' ? 'warning' : 'error'} variant="pastel" className="pointer-events-none mt-1">
                    Type {selectedRow?.logType} ({LOG_TYPE_MAP[selectedRow?.logType || '']})
                  </Button>
                </Box>
              </Box>
        ...
        {!loadingDetails && Object.keys(inventoryData).length > 0 && selectedRow?.logType !== '2' && selectedRow?.logType !== '3' && (
